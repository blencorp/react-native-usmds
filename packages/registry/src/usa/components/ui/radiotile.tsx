...
const radioTileVariants = cva('flex w-[329px] flex-col rounded-[2px] border-2', {
  variants: {
    variant: {
      default: '',
...
type RadioTileProps = ComponentPropsWithoutRef<typeof RadioGroupPrimitive.Item> &
...
const RadioTile = forwardRef<ElementRef<typeof RadioGroupPrimitive.Item>, RadioTileProps>(
  ({ label, description, variant = description ? 'withDescription' : 'default', className, ...props }, ref) => {
            ...
            <View className={cn(radioItemVariants({ state }), 'flex items-center justify-center')}>
              ...
              >
                <RadioGroupPrimitive.Indicator className="items-center justify-center">
                  <View
                    className={cn(
                      radioIndicatorVariants({ state: props.disabled ? 'disabled' : 'checked' })
                    )}
                  />
