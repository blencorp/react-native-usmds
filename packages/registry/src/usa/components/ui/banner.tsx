...
const FlagIcon = ({ size = 31 }) => {
  ...
  return (
    <Image
      source={{
        uri: "https://storage.googleapis.com/usmds-registry/assets/banner/us_flag_small.png",
      }}
...
const Banner = forwardRef<ElementRef<typeof View>, BannerProps>(
  (
    {
      className,
      variant,
      ...
      >
        <View className={cn(bannerVariants({ variant }))}>
          ...
          <View
            className={cn(
              "flex flex-col",
              isStandardApp ? "gap-3 flex-1" : "gap-0",
            )}
            pointerEvents="box-none"
          >
            <Text className={bannerTextVariants({ variant })}>
              {isStandardApp ? bannerContent.header : title}
            </Text>
            {isStandardApp && (
              <Pressable
