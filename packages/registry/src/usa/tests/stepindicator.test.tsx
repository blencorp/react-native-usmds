...
describe("StepIndicator", () => {
  it("renders correctly with basic props", () => {
    const { getByText } = render(
      <StepIndicator steps={3} currentStep={1} title="Test Step" />,
    );
  ...
  it("displays current step and total steps correctly", () => {
    const { getByText } = render(
      <StepIndicator steps={5} currentStep={3} title="Test Step" />,
    );
  ...
  it("renders with custom title", () => {
    ...
    const { getByText } = render(
      <StepIndicator steps={3} currentStep={1} title={customTitle} />,
    );
