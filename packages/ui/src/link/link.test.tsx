...
describe("Link", () => {
  ...
  it("renders external link with icon", () => {
    const { getByText, getByTestId } = render(
      <Link {...defaultProps} external testID="external-link" />
    );
  ...
  it("handles press events correctly", () => {
    ...
    const { getByTestId } = render(
      <Link {...defaultProps} onPress={onPress} testID="pressable-link" />
    );
  ...
  it("renders dark background variant correctly", () => {
    const { getByText } = render(
      <Link {...defaultProps} variant="dark-background" />
    );
    const linkText = getByText("Click me");
  ...
  it("handles focus state correctly", () => {
    const { getByTestId } = render(
      <Link {...defaultProps} testID="focus-link" focus />
    );
  ...
  it("combines visited and external states correctly", () => {
    const { getByTestId, getByText } = render(
      <Link {...defaultProps} visited external testID="visited-external-link" />
    );
  ...
  it("handles press state changes correctly", () => {
    const { getByTestId } = render(
      <Link {...defaultProps} testID="press-state-link" />
    );
