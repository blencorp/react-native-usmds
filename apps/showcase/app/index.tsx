...
export default function ComponentsScreen() {
  ...
  const flashListRef =
    React.useRef<React.ComponentRef<typeof FlashList<ShowcaseListItem>>>(null);
  ...
  const renderItem = React.useCallback(
    ({ item }: ListRenderItemInfo<ShowcaseListItem>) => (
      <Link
        href={{ pathname: "/components/[slug]", params: { slug: item.slug } }}
        asChild
      >
        <Link.Trigger>
          <Pressable
          ...
          >
            ...
            <Icon
              as={ChevronRight}
              className="mt-1 size-4 stroke-[1.5px] text-muted-foreground"
            />
          </Pressable>
        </Link.Trigger>
        <Link.Preview
