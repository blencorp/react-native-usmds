...
After running `npx usmds init`, you need to complete these manual steps:
...
```bash
npx usmds add button alert badge
```
...
```bash
npx usmds add alert badge button card
```
