    ...
    >
      {companies.map((company, index) => (
        <div key={index} className="mb-8">
          ...
          {company.description && (
            <div className="text-foreground text-lg">{company.description}</div>
          )}
        </div>
      ))}
      ...
      <div className="text-neutral-500">
        ...
        <p>
          Ad Hoc is a digital services company that emerged from the
          successful rescue of HealthCare.gov and helps federal government
          agencies deliver people-centric experiences that are efficient,
          accessible, reliable, and usable.{" "}
          <a
